    ...
    <div className="home">
      ...
      <form className="form home__form" onSubmit={handleSubmit(onSubmit)}>
        <div className="form__errors">
          {errors.location && position === null && !askingPosition && (
            <span className="form__errors--error">
              You must enter a location!
            </span>
          )}
          {!!loadingErrors.length && (
            <>
              {loadingErrors.map((err, idx) => (
                <li className="form__errors--error" key={idx}>
                  {err}
                </li>
              ))}
            </>
        ...
        <div className="form__flex">
          <div className="form__location">
            {position === null && !askingPosition && (
              <LocateArrow className="form__location--arrow" onClick={locate} />
            )}
            {askingPosition && <Loading className="form__location--loading" />}
            {!askingPosition && position !== null && (
              <Cancel
                className="form__location--cancel"
          ...
          <div className="form__inputs">
            ...
            {position && typeof position === "string" && (
              <p className="form__inputs--text">{position}</p>
            )}
          </div>
        </div>
